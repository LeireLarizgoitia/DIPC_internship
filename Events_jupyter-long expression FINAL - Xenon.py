...
def cross_section(T,Enu):
    ...
    dsigmadT = Gf**2 /(2*np.pi) * Qw**2 / 4 * F(Q2)**2 * M / (hbar_c_ke)**4 * (2 - 2*T *1E-3 / Enu  + (T *1E-3 / Enu)**2 - M *T * 1E-6 / Enu**2 ) #cm^2/keV
    return dsigmadT
...
def Plot_thresold():
    ...
    plt.legend()
    plt.savefig("Plot thresold change till Enu_mu_Xe_per kg year.png", format='png', dpi=1200,bbox_inches = 'tight')
    plt.show()
