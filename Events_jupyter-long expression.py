...
T_min= 0.9
T_max = 1/2 * (M*c**2/e*1e-3 + 2*Enu_max*1e3 - np.sqrt((M*c**2/e*1e-3)**2 + 4*Enu_max*1e3*(M*c**2/e*1e-3 - Enu_max*1e3)))
...
def cross_section(T,Enu):
    ...
    dsigmadT = (Gf*0.0389379)**2 /(2*np.pi) * QQ()**2 / 4 * F(Q2)**2 * M *c**2 * 1E-38/ e / hbar_c**2  *     (2 -T *1E-3 / Enu  + (T *1E-3 / Enu)**2 - M * c**2 * T * 1E-9 / (e*Enu**2) )
    return dsigmadT
...
def differential_events(T,a):
    ...
    Tnu_mu =  1/2 * (M*c**2/e*1e-3 + 2*Enu_mu*1e3 - np.sqrt((M*c**2/e*1e-3)**2 + 4*Enu_mu*1e3*(M*c**2/e*1e-3 - Enu_mu*1e3)))
    if (a==0):
        if (T<Tnu_mu):
...
def Plot_form_factor():
    ...
    FF4=[]
    FF2=[]
    FF0 = N/Qw * np.ones(nsteps+1,float)
...
def Plot_thresold():
    ...
    for j in range(0,nsteps+1):
        ...
        for i in range(0,nsteps+1):
            ...
            dNdT.append(normalization*(differential_events(TT[i],0) + differential_events(TT[i],1)+ differential_events(TT[i],2)))
    ...
    plt.ylabel(r'$$CE\nu NS \, \mathrm{nuclear \, recoils} \, in \, 3 \, years$$')
    plt.legend()
    #plt.savefig("Plot thresold change till Enu_mu_long exp_thres=0,9.png", format='png', dpi=1200,bbox_inches = 'tight')
    plt.show()
...
def Plot_counts_all_flavours_histogram(T_resol_antimu, T_resol_mu, T_resol_e, binss, T_thres,centres,Tbg,Nbg):

    n, bins, patches = plt.hist([T_resol_antimu,T_resol_e, T_resol_mu, Tbg], binss, density=False, stacked=True, label= [r'$$\bar{\nu}_{\mu}$$',r'$$\nu_{e}$$',r'$$\nu_{\mu}$$', r'$$N_{bg}$$'], color = ['lightgreen','yellowgreen','olivedrab','cornflowerblue'])
    Nbg.insert(0,0.0)
    plt.step(binss, np.sqrt(Nbg), linestyle='dashed' , label=r'$$\sqrt{N_{bg}}$$')
    ...
    plt.ylabel(r'$$Counts/bin $$')
    plt.legend()
    #plt.savefig('Plot counts with QF with nbg with sqrt and NO factor 10.png', format='png', dpi=1200,bbox_inches = 'tight')
    plt.show()
...
if QF!=1.:
    ...
    binss = []
    binss.clear()
    binss.append(T_thres) #starts at T_thresold shifted
    for t in centres:
        sigma = sigma0 * T_thres * np.sqrt(t/T_thres)
        binss.append(t + sigma)
